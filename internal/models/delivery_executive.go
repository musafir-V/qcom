...
type DeliveryExecutive struct {
	...
	// cleared for busy/offline. Used by the DEDutyIndex GSI so the assignment
	// cron (filter status=eligible) and on-duty metrics can query by store.
	DutyIndexKey string `json:"duty_index_key,omitempty" dynamodbav:"duty_index_key,omitempty"`
	// ScanDeadlineAt is leftover from the removed 15-minute sweep. Writes now
	// REMOVE it; the field remains so old items still unmarshal.
	ScanDeadlineAt string `json:"scan_deadline_at,omitempty" dynamodbav:"scan_deadline_at,omitempty"`
	// LastScan* stamp the most recent validated presence scan. Cron assignment
	// requires LastScanAt >= today's store opens_at.
